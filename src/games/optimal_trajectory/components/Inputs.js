...
const Inputs = (props) => {
  ...
  return (
    <div className="small mt-4 pt-2">
      {`${lang.radius}: ${radius}`}
      <Input className="custom-range border-0 mb-2" style={{ background: 'none' }} type="range" bsSize="sm" min="40" max="80" step="1" value={radius} name="radius" onChange={changeGameSettings} />
      {`${lang.count_of_balls}: ${ballsCount}`}
      <Input className="custom-range border-0 mb-2" style={{ background: 'none' }} type="range" bsSize="sm" min="1" max="10" step="1" value={ballsCount} name="ballsCount" onChange={changeGameSettings} />
