...
export class App extends BaseApp {
  ...
  generateResult = () => {
    const { ballsCount, targetBallsCount, targetBalls, selectedBalls, level, speed, radius } = this.state
    let trueBalls = 0
    let falseBalls = 0
    let clickedBalls = {}
  ...
  render() {
    ...
    return (
      <div className="w-100 h-100 d-flex">
        <Menu
          token={token}
          current_level={{ level, balls: ballsCount, speed, red_balls: targetBallsCount }}
          radius={radius}
          status={status}
